# 毫米波手势识别 - 方案二:轨迹图 + ResNet + CTC

## 📚 目录

- [基础知识](#基础知识)
- [整体思路](#整体思路)
- [模型架构](#模型架构)
- [环境配置](#环境配置)
- [使用教程](#使用教程)
- [实验设置](#实验设置)
- [代码结构](#代码结构)

---

## 基础知识

### 1. 轨迹重建

**为什么要重建轨迹?**
- 将雷达信号转换为直观的手势轨迹图像
- 模拟手写笔迹,更符合人类认知
- 便于使用成熟的计算机视觉模型处理

**三维轨迹提取:**
1. **Range FFT(距离FFT)**:提取目标距离信息
2. **Azimuth FFT(方位角FFT)**:提取水平方向角度
3. **Elevation FFT(俯仰角FFT)**:提取垂直方向角度

**轨迹处理流程:**
```
雷达信号 → FFT分析 → 3D坐标(距离,方位,俯仰) 
→ 卡尔曼滤波平滑 → PCA降维到2D → 轨迹图像
```

### 2. ResNet架构

**为什么选择ResNet?**
- ResNet是经典的卷积神经网络,在图像识别任务上表现优异
- **残差连接**:解决深层网络的梯度消失问题
- **预训练权重**:使用ImageNet预训练模型,加速收敛
- **多尺度特征**:融合不同层级的特征,提升识别精度

**ResNet的优势:**
- 成熟稳定,易于训练
- 支持多种深度(ResNet18/34/50)
- 强大的特征提取能力

---

## 整体思路

### 数据流程图

```
原始雷达数据 (.npy)
    │
    │ (帧数 × 108样本点 × 12通道)
    ↓
┌─────────────────────┐
│  FFT频谱分析        │
│  - Range FFT        │
│  - Azimuth FFT      │
│  - Elevation FFT    │
└─────────────────────┘
    │
    │ (帧数 × 3维坐标)
    ↓
┌─────────────────────┐
│  轨迹重建           │
│  - 噪声滤除         │
│  - 目标定位         │
│  - 卡尔曼平滑       │
└─────────────────────┘
    │
    │ (3D轨迹点序列)
    ↓
┌─────────────────────┐
│  PCA降维            │
│  投影到2D平面       │
└─────────────────────┘
    │
    │ (2D轨迹坐标)
    ↓
┌─────────────────────┐
│  轨迹图像生成       │
│  (PNG格式)          │
└─────────────────────┘
    │
    │ (灰度图像)
    ↓
┌─────────────────────┐
│  ResNet编码器       │
│  - 多尺度特征提取   │
│  - 特征融合         │
└─────────────────────┘
    │
    │ (时间帧 × 特征维度)
    ↓
┌─────────────────────┐
│  CTC解码            │
│  输出字符序列       │
└─────────────────────┘
    │
    ↓
   识别结果 (单词)
```

### 关键步骤说明

#### 步骤1:信号预处理
- 加载原始`.npy`文件
- 计算噪声基线(所有帧的平均值)
- 从每帧中减去噪声

#### 步骤2:3D轨迹提取
- **Range FFT**:使用Hann窗和零填充,提取180个距离bin
- **Azimuth FFT**:选择特定天线(0,1,2,3,8,9,10,11),计算方位角
- **Elevation FFT**:选择天线9和7,计算俯仰角
- 对每一帧定位目标,提取3D坐标(距离,方位,俯仰)

#### 步骤3:轨迹平滑与降维
- 使用卡尔曼滤波平滑轨迹,减少噪声
- 球坐标转换为笛卡尔坐标(x,y,z)
- PCA降维到2D平面,保留主要运动方向
- 调整轨迹方向,统一书写方向

#### 步骤4:轨迹图像生成
- 将2D轨迹绘制为黑色线条
- 自适应调整图像边界
- 保存为PNG格式的灰度图像

#### 步骤5:ResNet特征提取
- 加载轨迹图像(调整大小为128×256)
- ResNet提取多尺度特征(layer3 + layer4)
- 特征融合,保留空间信息(2行特征)
- 输出序列特征供CTC使用

#### 步骤6:CTC序列识别
- CTC层输出每个时间步的字符概率
- 使用贪心解码得到最终单词
- (可选)使用语言模型进行拼写纠正

---

## 模型架构

### 整体架构图

```
输入: (batch, 1, 128, 256) 灰度图
           │
           ↓
    ResNet Backbone
    ┌──────────────┐
    │  Conv1+BN+ReLU│
    │  MaxPool      │
    │  Layer1       │
    │  Layer2       │
    └──────────────┘
           │
     ┌─────┴─────┐
     ↓           ↓
  Layer3      Layer4
  (中层特征) (高层特征)
     │           │
     ↓           ↓
  Projection  Projection
  [D/2]        [D/2]
     │           │
     └─────┬─────┘
           ↓
      特征融合
    (Concatenate)
      [D通道]
           │
           ↓
     融合卷积
    (Conv+BN+ReLU)
           │
           ↓
   自适应池化(2行)
    [B, D, 2, W/8]
           │
           ↓
     序列展平
   [B, W/8, D*2]
           │
           ↓
      线性投影
    [B, W/8, D]
           │
           ↓
      CTC分类头
   (Dropout+Linear)
           │
           ↓
   [B, W/8, vocab_size]
```

### 详细参数配置

| 模块 | 参数 | 说明 |
|------|------|------|
| **输入图像** | 128×256 | 灰度图,标准化到[-1,1] |
| **ResNet类型** | ResNet50(默认) | 支持ResNet18/34/50 |
| **特征维度** | 512 | 编码器输出维度 |
| **多尺度融合** | Layer3+Layer4 | 中层+高层特征 |
| **空间池化** | (2, None) | 保留2行高度信息 |
| **序列长度** | W/8 | 约32时间步 |
| **Dropout** | 0.1 | 防止过拟合 |
| **词汇表** | 28个 | blank + unk + 26个字母 |

### ResNet Block详解

每个ResNet Layer包含多个Bottleneck/BasicBlock:

```
ResNet50 Bottleneck:
1. 1×1卷积降维 (channels → channels/4)
2. 3×3卷积提取特征
3. 1×1卷积升维 (channels/4 → channels)
4. 残差连接 + ReLU

特征下采样倍数:
- Layer1: 4倍 (stride=1)
- Layer2: 8倍 (stride=2)
- Layer3: 16倍 (stride=2)
- Layer4: 32倍 (stride=2)
```

### 增强特性

**多尺度特征融合:**
- Layer3提供更细粒度的局部特征
- Layer4提供更抽象的全局特征
- 上采样+拼接融合两路特征

**空间信息保留:**
- 保留2行高度信息而非1行
- 增强对字母细节的捕捉能力
- 特别适合区分形状相似的字母(如i/l, m/n)

---

## 环境配置

### 系统要求

- **操作系统**: Linux / macOS(不建议) / Windows
- **Python版本**: 3.8+
- **GPU**: NVIDIA GPU (推荐,支持CUDA 11.0+)
- **内存**: 至少16GB RAM
- **硬盘**: 至少10GB可用空间

### 依赖库安装

```bash
# 创建虚拟环境(推荐)
conda create -n mmPencil-trajectory python=3.8
conda activate mmPencil-trajectory

# 安装PyTorch(根据你的CUDA版本选择)
# CUDA 11.8版本
pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu118

# 或者CPU版本(不建议)
pip install torch torchvision torchaudio

# 安装其他依赖
pip install numpy
pip install tqdm
pip install pykalman
pip install scikit-learn
pip install editdistance
pip install Pillow
pip install matplotlib

# (可选)如果需要LLM拼写纠正
pip install vllm
```

---

## 使用教程

### 第一步:数据预处理(信号→轨迹图)

**脚本说明:** 第一个代码文件负责将原始毫米波信号转换为2D轨迹图像。

```bash
# 运行轨迹重建
cd NWPU_AIoT_Laboratory/
python trajectory-based_recognition/01_signal_to_trajectory.py
```

**输入数据格式:**
```
mmPencil_dataset/mmWave/
├── 200-Word/
│   ├── User-01/
│   │   ├── word1/
│   │   │   ├── w01.npy
│   │   │   ├── w02.npy
│   │   │   └── ...
│   │   └── ...
│   └── ...
└── ...
```

**输出数据格式:**
```
mmPencil_dataset/mmWave_Trajectory/
├── 200-Word_User-01_word1_w01.png  # 轨迹图像
├── 200-Word_User-01_word1_w02.png
└── ...
```

**处理流程说明:**
1. 加载`.npy`雷达数据
2. 执行Range/Azimuth/Elevation FFT
3. 提取每一帧的目标3D坐标
4. 卡尔曼滤波平滑轨迹
5. PCA降维到2D
6. 绘制并保存轨迹图像

**并行处理:**
- 使用多进程并行处理,加速转换
- 自动跳过已存在的文件
- 显示进度条

---

### 第二步:数据集划分

**脚本说明:** 第二个代码文件将数据划分为训练集和测试集,支持三种实验设置。

```bash
# 运行数据集划分
python trajectory-based_recognition/02_dataset_split.py
```

**三种实验设置:**

#### 1. 标准200词识别 (`200-Word.json`)
- **训练集**: 每个单词随机选20个样本 (200×20 = 4000样本)
- **测试集**: 每个单词剩余4个样本 (200×4 = 800样本)
- **目的**: 评估模型在见过的单词上的识别能力

#### 2. 零样本学习 (`zero-shot.json`)
- **训练集**: 150个单词的所有样本 (150×24 = 3600样本)
- **测试集**: 50个单词的所有样本 (50×24 = 1200样本)
- **目的**: 评估模型泛化到新单词的能力

#### 3. 跨用户识别 (`user-split.json`)
- **训练集**: User-01和User-02的所有数据
- **测试集**: User-03和User-04的所有数据
- **目的**: 评估模型跨用户的泛化能力

---

### 第三步:模型训练

**脚本说明:** 第三个代码文件实现完整的训练、测试和推理流程。

#### 基本训练命令

```bash
# 训练200词识别模型(ResNet50)
python trajectory-based_recognition/03_resnet_ctc_train.py --experiment 200word

# 训练零样本学习模型
python trajectory-based_recognition/03_resnet_ctc_train.py --experiment zero_shot

# 训练跨用户识别模型
python trajectory-based_recognition/03_resnet_ctc_train.py --experiment cross_user

# 使用ResNet18(更快,参数更少)
python trajectory-based_recognition/03_resnet_ctc_train.py --experiment 200word --encoder resnet18

# 使用ResNet34(平衡性能)
python trajectory-based_recognition/03_resnet_ctc_train.py --experiment 200word --encoder resnet34
```

#### 训练参数说明

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--experiment` | **必需** | 实验类型: 200word / zero_shot / cross_user |
| `--encoder` | resnet50 | ResNet类型: resnet18 / resnet34 / resnet50 |
| `--batch_size` | 16 | 批次大小 |
| `--learning_rate` | 1e-4 | 初始学习率 |
| `--num_epochs` | 20 | 训练轮数 |
| `--gpus` | 自动检测 | 使用的GPU数量 |
| `--config` | None | 额外配置文件路径 |

#### 高级配置示例

创建配置文件 `config.json`:
```json
{
  "batch_size": 32,
  "learning_rate": 5e-5,
  "num_epochs": 30,
  "feature_dim": 512,
  "freeze_backbone": false,
  "dropout": 0.2,
  "warmup_epochs": 5,
  "weight_decay": 1e-5
}
```

使用配置文件训练:
```bash
python trajectory-based_recognition/03_resnet_ctc_train.py \
  --experiment 200word \
  --config config.json
```

#### 训练结果

训练完成后,会生成以下文件:

1. **模型文件** (`.pth`)
```
trajectory-based_recognition/output/
└── mmwave_resnet50_ctc_model.pth  # 保存最佳模型
```

模型文件包含:
- `model_state_dict`: 模型权重
- `optimizer_state_dict`: 优化器状态
- `char_to_idx`: 字符到索引的映射
- `config`: 训练配置
- `best_test_acc`: 最佳测试准确率
- `epoch`: 训练轮数

2. **推理结果** (`.jsonl`)
```
trajectory-based_recognition/output/
├── inference_results_resnet50_ctc.jsonl  # 详细推理结果
└── stats_original_inference_results_resnet50_ctc.json  # 统计信息
```

3. **LLM输入文件** (`.jsonl`)
```
trajectory-based_recognition/output/
└── llm_input_inference_results_resnet50_ctc.jsonl  # 用于拼写纠正
```

---

### 第四步:模型评估

训练完成后,模型会自动在测试集上进行详细评估,并生成以下指标:

#### 评估指标说明

1. **单词准确率 (Word Accuracy)**
   - 定义:完全正确识别的单词数 / 总单词数
   - 公式:`Acc_word = N_correct / N_total`
   - 示例:预测"hello",真实"hello" → 正确 ✓

2. **字母准确率 (Character Accuracy)**
   - 定义:基于编辑距离的字符级准确率
   - 公式:`Acc_char = 1 - (总编辑距离 / 总字符数)`
   - 示例:预测"helo",真实"hello" → 编辑距离=1,字母准确率=4/5=0.8

3. **平均编辑距离 (Average Edit Distance)**
   - 定义:预测单词与真实单词之间的平均编辑操作次数
   - 包括:插入、删除、替换操作
   - 示例:
     - "hello" → "helo": 距离=1 (删除1个l)
     - "world" → "word": 距离=1 (删除1个l)

---

### 第五步:(可选)LLM拼写纠正

**脚本说明:** 第四个代码文件使用大语言模型对识别结果进行拼写纠正。

#### 为什么需要拼写纠正?

- CTC模型可能产生拼写错误(如"helo" → "hello")
- 语言模型可以利用英语单词知识进行纠正
- 进一步提升识别准确率

#### 使用vLLM进行批量纠正

```bash
# 基本使用
python trajectory-based_recognition/04_llm_correction.py \
  --input trajectory-based_recognition/output/llm_input_inference_results_resnet50_ctc.jsonl \
  --output trajectory-based_recognition/output/llm_output_inference_results_resnet50_ctc.jsonl \
  --model /path/to/your/llm/model

# 指定批次大小和参数
python trajectory-based_recognition/04_llm_correction.py \
  --input trajectory-based_recognition/output/llm_input_inference_results_resnet50_ctc.jsonl \
  --output trajectory-based_recognition/output/llm_output_inference_results_resnet50_ctc.jsonl \
  --model /path/to/your/llm/model \
  --batch-size 256 \
  --max-tokens 10 \
  --temperature 0.0
```

#### 参数说明

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--input` | **必需** | LLM输入文件路径 |
| `--output` | **必需** | LLM输出文件路径 |
| `--model` | models/Qwen3-0.6B | 语言模型路径 |
| `--batch-size` | 256 | 批次大小 |
| `--max-tokens` | 10 | 最大生成token数 |
| `--temperature` | 0.0 | 采样温度(0.0表示贪心) |

#### 纠正结果示例

```
================================================================================
纠正结果统计
================================================================================
总样本数: 800
原始正确数: 720 (0.9000, 90.00%)
纠正后正确数: 760 (0.9500, 95.00%)
准确率提升: +0.0500 (+5.00%)
改进样本数: 45
退化样本数: 5
净提升: 40
================================================================================

纠正示例(前20个):
真实         原始预测      LM纠正       原始   纠正  
------------------------------------------------------------
hello        helo         hello        ✗      ✓     
world        wrold        world        ✗      ✓     
apple        aple         apple        ✗      ✓     
...
```

#### 输出文件

1. **纠正结果** (`llm_output_*.jsonl`)
```json
{
  "custom_id": "request-0",
  "original_prediction": "helo",
  "ground_truth": "hello",
  "lm_output": "hello",
  "corrected_word": "hello",
  "original_correct": false,
  "corrected_correct": true
}
```

2. **统计信息** (`stats_corrected_*.json`)
```json
{
  "total_samples": 800,
  "original_correct": 720,
  "corrected_correct": 760,
  "original_accuracy": 0.9000,
  "corrected_accuracy": 0.9500,
  "accuracy_improvement": 0.0500,
  "improved_samples": 45,
  "degraded_samples": 5,
  "net_improvement": 40
}
```

3. **原始LLM输出** (`raw_llm_output_*.jsonl`) - 用于调试

---

## 实验设置（同方案一）

### 三种实验的详细说明

#### 实验1:标准200词识别

**目标**: 评估模型在已知词汇上的识别能力

**数据划分**:
- 200个单词,每个单词24个样本
- 训练集:每个单词20个样本(共4000样本)
- 测试集:每个单词4个样本(共800样本)

**训练命令**:
```bash
python trajectory-based_recognition/03_resnet_ctc_train.py --experiment 200word
```

---

#### 实验2:零样本学习

**目标**: 评估模型泛化到新单词的能力

**数据划分**:
- 训练集:150个单词的所有样本(3600样本)
- 测试集:50个新单词的所有样本(1200样本)
- **关键**: 测试集的单词在训练时从未见过

**挑战**:
- 模型需要学习字母级别的特征,而不是记忆单词
- 考验模型的泛化能力
- 轨迹图的视觉相似性可能带来额外挑战

**适用场景**:
- 开放词汇表的手势识别
- 需要识别任意英文单词的应用

**训练命令**:
```bash
python trajectory-based_recognition/03_resnet_ctc_train.py --experiment zero_shot
```

---

#### 实验3:跨用户识别

**目标**: 评估模型跨用户的泛化能力

**数据划分**:
- 训练集:User-01和User-02的所有数据
- 测试集:User-03和User-04的所有数据
- **关键**: 不同用户的书写习惯、手势速度可能不同

**挑战**:
- 用户间的手势差异
- 书写风格的个性化
- 轨迹粗细、速度的变化

**适用场景**:
- 多用户共享的手势识别系统
- 无需用户注册的应用

**训练命令**:
```bash
python trajectory-based_recognition/03_resnet_ctc_train.py --experiment cross_user
```

---

## 代码结构

### 文件组织

```
NWPU_AIoT_Laboratory/
├── mmPencil_dataset/
│   ├── mmWave/                     # 原始雷达数据
│   │   └── 200-Word/
│   │       └── User-XX/
│   │           └── word/
│   │               ├── wXX.npy    # (帧数, 108, 12)
│   │               └── ......
│   └── mmWave_Trajectory/          # 轨迹图像
│       ├── 200-Word_User-XX_word_wXX.png  # 128×256 灰度图
│       └── ......
│
├── trajectory-based_recognition/
│   ├── dataset_index/              # 数据集划分JSON文件
│   │   ├── 200-Word.json
│   │   ├── zero-shot.json
│   │   └── user-split.json
│   └── output/                     # 训练输出
│       ├── mmwave_resnet50_ctc_model.pth
│       ├── inference_results_resnet50_ctc.jsonl
│       ├── llm_input_*.jsonl
│       ├── llm_output_*.jsonl
│       ├── stats_*.json
│       └── ......
│
├── 01_signal_to_trajectory.py      # 信号→轨迹图
├── 02_dataset_split.py             # 数据集划分
├── 03_resnet_ctc_train.py          # 训练主程序
├── 04_llm_correction.py            # LLM拼写纠正
└── README.md                       # 本文档
```
---

**祝学业顺利!🎉🎉🎉**

---