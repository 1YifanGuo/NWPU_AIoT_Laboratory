...
def save_trajectory(trajectory_2d: np.ndarray, file_path: str, dataset_root: str) -> None:
    """
    Save a 2D trajectory as a PNG image.

    The label is extracted from the word directory in the input file path.
    The image is saved with a structured naming convention.

    Parameters:
        trajectory_2d (np.ndarray): Array of shape (n_frames, 2), representing 2D trajectory points.
        file_path (str): Path to the original .npy file.
        dataset_root (str): Root directory of the dataset (e.g., "mmPencil_dataset/mmWave").
    """
    ...
    if len(parts) < 4:
        print(f"[Error] Unexpected path format: {file_path}")
        return

    # Parse path based on experiment type
    ...
    # Extract trial name and create output filename
    trial = trial_filename.replace('.npy', '')
    output_name = f"{experiment}_{subject}_{word}_{trial}.png"

    # Use word as the label
    label = word

    # Define output directories
    output_dir = "mmPencil_dataset/mmWave_Trajectory"

    # Create output path
    os.makedirs(output_dir, exist_ok=True)

    png_save_path = os.path.join(output_dir, output_name)
...
def process_single_file(file_path: str, dataset_root: str, n_sample: int, n_channel: int,
                        slope: float, sample_rate: float, N: int, c: float) -> None:
    """
    ...
    # Parse path to create output filename
    parts = Path(file_path).parts
    if len(parts) < 4:
        print(f"[Error] Unexpected path format: {file_path}")
        return None
    ...
    try:
        ...
        save_trajectory(trajectory_2d, file_path, dataset_root)
